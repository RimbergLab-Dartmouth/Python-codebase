...
ax.set_yticklabels([r'-1/2',r'0',r'1/2'])
ax.set_xticks([0,1/2,1])
ax.set_xticklabels(['0','1/2','1'])

ax.tick_params(labelsize=ticklabelsize,length=ticklength,width=tickwidth)
ax.grid(linestyle=grid_linestyle,alpha=grid_alpha,linewidth=grid_linewidth)


#plt.tight_layout(pad=figpad)
#fig.subplots_adjust(bottom=bottom_adjust,left=left_adjust)
plt.tight_layout(pad=fig_pad,w_pad = fig_wpad,h_pad=fig_hpad,rect=[0.2,0.0,1.05,0.99])
fig.subplots_adjust(left=left_adjust)


plt.savefig(figs_dir+'s11_complex_trajectory.'+file_extension,transparent=True,dpi=dpi)
plt.show()