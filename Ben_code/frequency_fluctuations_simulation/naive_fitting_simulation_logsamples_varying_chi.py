...
def s21_model(df,sigma):
    ...
    term2 = ((-2.0j)/np.sqrt(np.pi))*sp.special.dawsn(argument)
    s21 = factor*(term1+term2)
    return s21

def s21_naive(df, xi, g_tot):
    s21 = (-2*np.sqrt(xi))/((1+xi)*(1+((2j*df)/g_tot)))
    return s21
...
labels = [r'$\kappa_{\mathrm{tot}}^{\prime}/\kappa_{\mathrm{tot}}$',r'$\xi^{\prime}/\xi$']
...
plt.tight_layout()
plt.savefig(figs_dir+'s21_fitting_simulation_logsamples_varying_xi.'+file_extension,transparent=True)
plt.show()
